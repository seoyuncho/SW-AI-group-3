...
if st.session_state.logged_in: # 로그인 시 다음 페이지로 이동
    ...
    if st.session_state.is_first: # 첫 방문 시 사전 정보 입력 페이지로 이동
        ...
        if st.session_state.main_page == 1:
            ...
            params["nx"] = x
            params["ny"] = y
            url = f"http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtFcst?serviceKey={serviceKey}&numOfRows=500&pageNo=10&dataType=json&base_date={base_date}&base_time=2300&nx={x}&ny={y}"
